...
it('returns never for an emitter without any events', () => {
  {
    ...
    expectTypeOf<Emitter.PublicEventTypes<typeof emitter>>().toBeNever()
  }
...
it('returns a single event type for an emitter with a single event', () => {
  ...
  expectTypeOf<
    Emitter.PublicEventTypes<typeof emitter>
...
it('returns a union of all event types for an emitter with many events', () => {
  ...
  expectTypeOf<Emitter.PublicEventTypes<typeof emitter>>().toEqualTypeOf<
    'greeting' | 'handshake'
  >()
})
